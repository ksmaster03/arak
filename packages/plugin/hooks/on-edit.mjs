...
function parseCatalog(text) {
  ...
  if (obj["controller"] !== void 0)
    catalog.controller = obj["controller"];
  if (obj["access"] !== void 0)
    catalog.access = obj["access"];
  if (Array.isArray(obj["securityMeasures"])) {
    catalog.securityMeasures = obj["securityMeasures"].map(String);
  }
  return { catalog, problems };
}
function readPurposes(value, problems) {
  ...
  const out = [];
  const legal = new Set(LEGAL_BASES);
  for (const [i, item] of value.entries()) {
    ...
    const p = item;
    const key = typeof p["key"] === "string" ? p["key"] : void 0;
    if (key === void 0) {
...
function readFields(value, problems) {
  if (value === void 0 || value === null)
    return [];
  if (!Array.isArray(value)) {
  ...
  for (const [i, item] of value.entries()) {
    ...
    const f = item;
    const id = typeof f["id"] === "string" ? f["id"] : void 0;
    if (id === void 0) {
...
function serializeCatalog(catalog, previous) {
  const doc = previous !== void 0 && previous.trim().length > 0 ? (0, import_yaml.parseDocument)(previous) : freshDocument(catalog);
  if (doc.get("version") === void 0)
    doc.set("version", 1);
...
function freshDocument(catalog) {
  const doc = new import_yaml.Document({
    version: 1,
...
function blockify(node) {
  (0, import_yaml.visit)(node, {
    Collection(_key, item) {
      if ((0, import_yaml.isCollection)(item) && item.items.length > 0)
        item.flow = false;
    }
  });
}
function writeFields(doc, fields) {
...
function sortKeys(node) {
  ...
  node.items.sort((a, b) => (rank.get(keyOf(a)) ?? FIELD_KEYS.length) - (rank.get(keyOf(b)) ?? FIELD_KEYS.length));
}
function compact(field) {
  const out = {};
  for (const key of FIELD_KEYS) {
    const value = field[key];
    if (value === void 0)
...
var import_yaml2, ALL_CATEGORIES;
...
function parseAnnotation(docLines) {
  ...
  if (match === null)
    return { annotation: null, errors };
  const kind = match[1] === "not-" ? "not-pii" : "pii";
  const body = match[2] ?? "";
  const raw = match[0];
  const annotation = { kind, raw };
  ...
  for (const [index, token] of tokens.entries()) {
    ...
    switch (key) {
      ...
      case "purpose":
      case "purposes":
        annotation.purposes = value.split(/[;|+]/).map((s) => s.trim()).filter((s) => s.length > 0);
        break;
      case "retention":
  ...
  if (kind === "not-pii" && (annotation.category !== void 0 || annotation.purposes !== void 0)) {
...
function parsePrismaSchema(text, file) {
...
function relationScalarNames(block) {
  ...
  for (const field of block.fields) {
    if (/@id\b/.test(field.attributes))
      names.delete(field.name);
  }
  return names;
}
function isRelationType(typeName, blockKinds) {
  if (SCALAR_TYPES.has(typeName))
    return false;
  if (typeName.startsWith("Unsupported("))
    return false;
...
function readPrismaSchemas(inputs) {
  ...
  for (const schema of schemas) {
    for (const block of schema.blocks)
      blockKinds.set(block.name, block.kind);
  }
  const fields = [];
  const problems = [];
  for (const schema of schemas) {
    for (const block of schema.blocks) {
      ...
      for (const field of block.fields) {
        ...
        const isRelation = isRelationType(field.typeName, blockKinds) || foreignKeys.has(field.name);
        if (isRelation && annotation !== null) {
          problems.push({
            level: "warning",
            id,
            message: "\u0E1F\u0E34\u0E25\u0E14\u0E4C\u0E19\u0E35\u0E49\u0E40\u0E1B\u0E47\u0E19\u0E04\u0E27\u0E32\u0E21\u0E2A\u0E31\u0E21\u0E1E\u0E31\u0E19\u0E18\u0E4C \u0E44\u0E21\u0E48\u0E43\u0E0A\u0E48\u0E15\u0E31\u0E27\u0E02\u0E49\u0E2D\u0E21\u0E39\u0E25 \u2014 \u0E43\u0E2B\u0E49\u0E21\u0E32\u0E23\u0E4C\u0E01\u0E17\u0E35\u0E48\u0E1F\u0E34\u0E25\u0E14\u0E4C\u0E1B\u0E25\u0E32\u0E22\u0E17\u0E32\u0E07\u0E41\u0E17\u0E19",
            file: schema.file,
            line: field.line
          });
        }
...
function loadProject(root, options) {
  ...
  for (const file of config.sources.prisma) {
    ...
    try {
      inputs.push({ file, text: readFileSync2(full, "utf8") });
    } catch {
  ...
  let previousText;
  let existing = emptyCatalog();
  const problems = [...read.problems];
  if (existsSync(catalogPath)) {
    previousText = readFileSync2(catalogPath, "utf8");
...
function findProjectRoot(filePath, cwd) {
  ...
  while (dir.startsWith(boundary)) {
    if (existsSync2(join3(dir, CONFIG_NAME))) return dir;
    const parent = dirname(dir);
    if (parent === dir) break;
    dir = parent;
  }
  return existsSync2(join3(boundary, CONFIG_NAME)) ? boundary : null;
}
function toRepoPath(root, filePath) {
  return relative2(root, filePath).split(sep2).join("/");
}
async function loadArak() {
  return Promise.resolve().then(() => (init_project(), project_exports));
}
function describePurposes(catalog) {
  if (catalog.purposes.length === 0) {
    return "  (\u0E22\u0E31\u0E07\u0E44\u0E21\u0E48\u0E21\u0E35\u0E27\u0E31\u0E15\u0E16\u0E38\u0E1B\u0E23\u0E30\u0E2A\u0E07\u0E04\u0E4C\u0E43\u0E19\u0E41\u0E04\u0E15\u0E15\u0E32\u0E25\u0E47\u0E2D\u0E01\u0E40\u0E25\u0E22 \u2014 \u0E15\u0E49\u0E2D\u0E07\u0E43\u0E2B\u0E49\u0E40\u0E08\u0E49\u0E32\u0E02\u0E2D\u0E07\u0E23\u0E30\u0E1A\u0E1A\u0E40\u0E1E\u0E34\u0E48\u0E21\u0E01\u0E48\u0E2D\u0E19)";
  }
  return catalog.purposes.map((p) => `  - ${p.key} \u2014 ${p.label} (\u0E10\u0E32\u0E19\u0E17\u0E32\u0E07\u0E01\u0E0E\u0E2B\u0E21\u0E32\u0E22: ${p.legalBasis}, \u0E40\u0E01\u0E47\u0E1A ${p.retention})`).join("\n");
}
...
async function main() {
  ...
  const pending = run.catalog.fields.filter(
    (field) => field.status === "unmarked" && field.orphaned !== true && field.source.file === repoPath
  );
  if (pending.length === 0) quiet();
  const lines = pending.sort((a, b) => a.source.line - b.source.line).map((field) => {
    const guess = field.category === void 0 ? "" : ` \u2014 \u0E15\u0E31\u0E27\u0E40\u0E14\u0E32\u0E27\u0E48\u0E32\u0E19\u0E48\u0E32\u0E08\u0E30\u0E40\u0E1B\u0E47\u0E19 ${field.category}` + (field.confidence === void 0 ? "" : ` (\u0E40\u0E0A\u0E37\u0E48\u0E2D\u0E21\u0E31\u0E48\u0E19 ${field.confidence})`);

...
| ทำแล้ว | ยังไม่ทำ |
|---|---|
| สคีมาแคตตาล็อกครบตาม ม.39 | ฮุก Claude Code ที่ปิดวงในเทิร์นเดียว |
| ตัวอ่านสคีมา Prisma + `@pii` | ตัวสร้างเอกสาร RoPA (.docx/.xlsx) |
| เขียนแคตตาล็อกกลับโดยคอมเมนต์ไม่หาย | กฎ Semgrep ห้าม PII ไหลลง log |
| ตัวเดาจากชื่อฟิลด์ 35 กฎ | MCP server อ่านไฟล์แบบปิดบัง (ด้าน B) |
| ตัวตรวจค่าจริงของไทย 12 ชนิด | ตัวอ่าน OpenAPI และ TypeScript type |
| ตัวปิดบังที่ให้ตัวแทนคงที่และแปลงกลับได้ | ชื่อไทยที่ไม่มีคำนำหน้า |
| `arak init` / `sync` / `status` / `scan` | |
